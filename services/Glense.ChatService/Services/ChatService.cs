...
public class ChatService : IChatService
{
    ...
    public async Task<PagedResponse<MessageDto>> GetMessagesAsync(Guid chatId, Guid? cursor, int pageSize, CancellationToken ct = default)
    {
        ...
        var dtos = items.Select(m => new MessageDto(m.Id, m.ChatId, m.UserId, m.Username, m.Sender.ToString().ToLower(), m.Content, m.CreatedAtUtc));
        var next = hasMore ? items.Last().Id : (Guid?)null;
        return new PagedResponse<MessageDto>(dtos, next);
    }

    public async Task<MessageDto> CreateMessageAsync(Guid chatId, Guid userId, string username, CreateMessageRequest req, CancellationToken ct = default)
    {
        ...
        var m = new Message { Id = Guid.NewGuid(), ChatId = chatId, UserId = userId, Username = username, Content = req.Content, Sender = sender, CreatedAtUtc = DateTime.UtcNow };
        _db.Messages.Add(m);
        await _db.SaveChangesAsync(ct);
        return new MessageDto(m.Id, m.ChatId, m.UserId, m.Username, m.Sender.ToString().ToLower(), m.Content, m.CreatedAtUtc);
    }
